...
сравненные по качеству с промышленными библиотеками. Проект создан для того,
чтобы понять, *почему* стандартные реализации работают именно так, а не только
...
Реализация градиентного бустинга в стиле CatBoost поверх деревьев регрессии.
...
Качество модели отличается от LightGBM всего на **0.7–1.5 процентных пункта по ROC-AUC**. Основная оставшаяся разница обусловлена использованием в LightGBM поиска разбиений по гистограммам и роста деревьев по листьям (leaf-wise growth), что не входит в задачи данного проекта.
